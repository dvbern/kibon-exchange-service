...
CREATE TABLE anmeldungmodul (
    ...
    anmeldung_id  VARCHAR(255) NOT NULL
...
CREATE INDEX clientanmeldung_idx1
	ON clientanmeldung(client_clientname, active, since, id);

CREATE INDEX anmeldung_idx1
...
$$
BEGIN
INSERT INTO clientanmeldung (id, active, client_clientname, client_institutionid, anmeldung_id, since)
...
BEGIN
INSERT INTO clientanmeldung (id, active, client_clientname, client_institutionid, anmeldung_id, since)
	(SELECT nextval('clientanmeldung_id_seq'), new.active, new.clientname, new.institutionid, am.id, new
		.grantedsince
	 FROM anmeldung am
	 WHERE am.institutionid = new.institutionid
	 ORDER BY new.grantedsince, am.id);
RETURN new;
END;
$$;
